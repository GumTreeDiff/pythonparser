...
_input_dir = os.path.join(RESOURCE_FOLDER_PATH, 'pythonparser_3')
...
def test_produce_valid_xml(cases_dirs: str) -> None:
    """
...
def test_non_valid_python_code(test_input_dirs: str) -> None:
    """
...
def test_correct_token_params(token_params_sample) -> None:
    input_file_paths = os.path.join(_input_dir, 'case_small_code_snippets/small_3.py')
    (expected, xml_element) = token_params_sample
    root = get_xml_tree(input_file_paths)
...
def test_all_tokens_are_accounted_for(tokens_to_cnt) -> None:
    """
    ...
    root = get_xml_tree(input_file_path)
    result = count_token_occurrence(root, expected.keys())
    assert result == expected
...
def test_merge_expr_context_and_ops_into_type(merged_tokens_to_cnt) -> None:
    """
